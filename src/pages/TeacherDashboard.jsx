...
import StudentDetailModal from '../components/StudentDetailModal'; // <--- IMPORT

ChartJS.register(CategoryScale, LinearScale, PointElement, LineElement, Title, Tooltip, Legend, Filler);

const TeacherDashboard = () => {
  const [loading, setLoading] = useState(true);
  const [stats, setStats] = useState({ totalStudents: 0, pendingUpdates: 0, highRiskCount: 0, updatesDone: '0%' });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const students = currentTeacherId ? allStudents.filter(s => s.teacher_id === parseInt(currentTeacherId)) : []; 
          ...
          if (history && history.length > 0) {
            ...
            if (latest.risk_level === 'High' || latest.risk_level === 'Medium') {
                riskList.push({
                    id: student.id,
                    name: student.name,
                    class: student.roll_number,
                    issue: latest.risk_reasons, // Keep raw or parsed
  ...
  const chartOptions = { responsive: true, plugins: { legend: { position: 'top' } }, scales: { y: { min: 60, max: 100 } } };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
            ...
            <div className="p-3 rounded-lg bg-yellow-100 text-yellow-600"><Clock size={24} /></div>
          </div>
          <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
            <div>
            ...
            <div className="p-3 rounded-lg bg-red-100 text-red-600"><AlertTriangle size={24} /></div>
          </div>
          <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
            <div>
            ...
            <div className="p-3 rounded-lg bg-green-100 text-green-600"><CheckCircle size={24} /></div>
          </div>
      </div>
          ...
          <div className="space-y-4">
            {atRiskStudents.length === 0 ? (
                ...
                <div key={student.id} className="p-4 border border-gray-100 rounded-lg hover:shadow-md transition-shadow bg-gray-50">
                    ...
                    <p className="text-xs text-gray-600 mb-3 line-clamp-1">{student.issue}</p>
